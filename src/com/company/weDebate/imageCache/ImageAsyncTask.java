package com.company.weDebate.imageCache;
...
/*
 * Attention , I have changed the Policy of ThreadPool into ThreadPoolExecutor.DiscardOldestPolicy
 *  so , the oldest of mission will be discard . 
 */
...
public abstract class ImageAsyncTask<Params, Progress, Result> {
	...
	private static final ThreadPoolExecutor sExecutor = new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, KEEP_ALIVE, TimeUnit.SECONDS, sWorkQueue, sThreadFactory, new ThreadPoolExecutor.DiscardOldestPolicy() {
		...
		public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
			System.out.println("sExecutor + " + r);
		}
	});
		...
		mFuture = new FutureTask<Result>(mWorker) {
			...
			protected void done() {
				...
				try {
					...
					e.printStackTrace();
					ILog.logException(ImageAsyncTask.class, e);
				} catch (ExecutionException e) {
					throw new RuntimeException("An error occured while executing doInBackground()", e.getCause());
				} catch (CancellationException e) {
					message = sHandler.obtainMessage(MESSAGE_POST_CANCEL, new AsyncTaskResult<Result>(ImageAsyncTask.this, (Result[]) null));
				...
				message = sHandler.obtainMessage(MESSAGE_POST_RESULT, new AsyncTaskResult<Result>(ImageAsyncTask.this, result));
				message.sendToTarget();
			}
		};
	}
	 ...
	 * {@link #doInBackground} or null if the task was cancelled or an exception occured.
	 * 
	 * @param result
